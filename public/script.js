document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateRemoteCursor(socketId, range) {
        ...
        cursorElement.innerHTML = `
            <div class="ql-cursor-caret" style="background-color: ${userColors[socketId] || getRandomColor()
            }"></div>
            <div class="ql-cursor-name" style="background-color: ${userColors[socketId] || getRandomColor()
            }">
                ${document.querySelector(
                `.user-item:nth-child(${Object.keys(userColors).indexOf(socketId) + 1
                }) span`
            )?.textContent || "User"
            }
            </div>
    ...
    function fetchDocumentHistory() {
        ...
        fetch(`/api/documents/${currentDocumentId}/history`)
            .then(response => response.json())
            .then(data => {
    ...
    function displayDocumentHistory() {
        ...
        const sortedHistory = [...documentHistory].sort((a, b) => b.timestamp - a.timestamp);
        ...
        sortedHistory.forEach(change => {
            ...
            if (change.delta.ops) {
                const previewText = change.delta.ops.map(op => {
                    if (op.insert) return `Added: "${typeof op.insert === 'string' ? op.insert : '[object]'}"`;
                    if (op.delete) return `Deleted ${op.delete} characters`;
                    if (op.retain) return `Kept ${op.retain} characters${op.attributes ? ' with formatting' : ''}`;
                    return '';
                }).join(', ');
