...
export const config = {
  ...
  expressPort: Number(process.env.EXPRESS_PORT) || (isTest ? 3003 : 3000),
  corsOrigin:
    process.env.CORS_ORIGIN ||
    (isTest
      ? `http://localhost:${Number(process.env.EXPRESS_PORT) || 3000}`
      : "*"),
...
console.log(
  `Socket port: ${config.socketPort}, Express port: ${config.expressPort}`
);
